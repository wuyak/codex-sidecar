...
import { renderInlineMarkdown, smartJoinParts } from "./inline";
import { isTableSeparatorLine, parseTableAlign, renderTable, splitTableRow } from "./table";
...
export function renderMarkdown(md: unknown): string {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (t.includes("|") && (i + 1) < lines.length && isTableSeparatorLine(String(lines[i + 1] ?? ""))) {
      ...
      const aligns = splitTableRow(String(lines[i + 1] ?? ""))
        .map(parseTableAlign)
    ...
    if (h) {
      flushPara();
      flushList();
      const level = Math.min(3, Math.max(1, String(h[1] || "").length));
