...
- **可选锁定跟随**：侧栏“锁定”开关开启时，点击会话会把**主跟随**固定到该会话（避免自动跳转）；同时仍会后台并行 tail 最近 N 个会话文件，新会话可持续出现在 sidebar。
- **多会话不串线**：在 `all` 视图下，每条消息会显示所属会话标识（来源于 rollout 文件时间戳 + thread_id 片段），降低“多会话混在一起像错乱”的困惑。
- **未读提醒**：新输出会在侧栏会话 tab 与右下角 🔔 显示未读数；点击 🔔 会跳到“最近未读”的会话并滚动到底部；到达底部后再点一次 🔔 清除未读（停止提醒）；可在设置中选择提示音或关闭。
...
### 会话管理（UI）
- 导出：侧栏顶部 `⤓` 可将当前会话导出为 Markdown（随当前 `快速浏览` 模式选择“精简/全量”）
- 隐藏：侧栏顶部 `🙈` 可隐藏当前会话（仅本机 localStorage，不会删除任何 `rollout-*.jsonl`）
- 显示隐藏：侧栏顶部 `👁` 可临时显示被隐藏的会话（便于找回/切换）
