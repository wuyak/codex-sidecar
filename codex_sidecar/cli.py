...
def _parse_args(argv):
    ...
    p.add_argument(
        ...
        help="Sidecar 配置目录（默认: ./config/sidecar，位于当前目录；可用 --config-home 覆盖）",
    )
...
def main(argv=None) -> int:
    ...
    if not args.no_server:
        ...
        try:
            ...
            lock_path = lock_dir / f"codex_sidecar.{args.port}.lock"
    ...
    if do_restart:
        cmd = [sys.executable, "-m", "codex_sidecar"] + raw_argv
        try:
