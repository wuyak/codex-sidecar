...
- UI 控制层：`ui/app/control/wire.js` 作为事件 wiring 入口，按功能域拆分到 `ui/app/control/wire/*`（例如 `ui_hints.js`、`import_dialog.js`、`secrets.js`、`sfx.js`、`export_prefs_panel.js`、`bookmark_drawer.js`），降低单文件耦合与复杂度。
