...
- UI 设置抽屉：常用配置 + “高级选项”折叠；保存配置/调试信息；皮肤切换（默认/柔和/对比/扁平/深色，仅影响前端观感，本机记忆）；字体大小/按钮大小（本机记忆，实时预览，不弹出额外提示）
...
- UI 会话切换：底部为“浏览器标签栏”式会话标签（横向滚动，始终可见当前会话）；标签栏默认右对齐、按内容自适应宽度（超过上限再横向滚动）；长按标签重命名；`×` 临时关闭监听（该会话有新输出时会自动回到标签栏）；右键可将会话从标签栏移除/恢复；右下角菜单按钮打开“会话管理”抽屉（四按钮：重命名/导出/监听/清除；导出按钮长按打开“导出设置”弹窗）
