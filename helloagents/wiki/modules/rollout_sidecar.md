...
- UI 导出层：`ui/app/export.js` 中与 tool_call 解析/归一化相关的逻辑抽离到 `ui/app/export/tool_calls.js`，quick blocks 读取/清洗抽离到 `ui/app/export/quick_blocks.js`，导出文件名/下载相关逻辑抽离到 `ui/app/export/naming.js`、`ui/app/export/download.js`，降低单文件复杂度（行为保持不变）。
