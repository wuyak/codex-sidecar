...
- UI 会话切换：右侧中部为“会话书签轨”（1~6 个，圆形书签，不占用对话区宽度）；鼠标悬停或键盘聚焦进入书签轨时，会弹出“完整会话列表”面板（覆盖式，不推动主内容）；列表项展示短时间戳（MM-DD HH:MM）+ shortId，hover 提示完整时间戳 + shortId；单击切换会话；长按在列表项原位重命名（Enter/失焦提交，Esc 取消）
