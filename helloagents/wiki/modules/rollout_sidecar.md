...
- UI 会话切换：底部为“浏览器标签栏”式会话标签（横向滚动，始终可见当前会话）；标签宽度按内容自适应增长，超过上限后才截断并显示省略号；**标签顺序保持稳定（不会随新消息重排）**，并尽量保持横向滚动位置，避免 SSE 频繁刷新导致“来回跳”；长按标签重命名；`×` 临时关闭监听（该会话有新输出时会自动回到标签栏）；右键可将会话从标签栏移除/恢复；右下角菜单按钮打开“会话管理”抽屉（四按钮：重命名/导出/监听/清除；导出按钮长按打开“导出设置”弹窗）
