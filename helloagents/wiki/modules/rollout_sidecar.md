...
- UI 设置抽屉：常用配置 + “高级选项”折叠；精简显示设置（可自定义精简模式下显示的内容块，长按 ⚡ 快速进入）；保存配置/调试信息；皮肤切换（默认/柔和/对比/扁平/深色，仅影响前端观感，本机记忆）；字体大小/按钮大小（本机记忆）
...
- UI 会话切换：底部为“浏览器标签栏”式会话标签（横向滚动，始终可见当前会话）；标签栏默认右对齐、按内容自适应宽度（超过上限再横向滚动）；长按标签重命名；`×` 临时关闭监听（该会话有新输出时会自动回到标签栏）；右键可将会话从标签栏移除/恢复；右下角菜单按钮打开“会话管理”抽屉（已关闭监听列表/导出/重命名/监听开关）
  - 选中即锁定跟随（pin-on-select）：开启时切换会话会向后端发送 `follow=pin`；关闭时仅切换视图并释放为 `follow=auto`，避免“监听跑偏/新会话不出现”的错觉
- UI 未读提醒：回答输出/审批提示计入未读；未读数显示在对应会话标签徽标上；切换到会话时自动清空该会话未读
...
- UI 以旁路查看/复制为主；如需归档可在“会话管理”抽屉内导出为 Markdown（下载到本机文件，文件名使用会话显示名；可选精简/全量、思考译文/原文），且不提供任何“在浏览器里触发本机执行”的控制入口（避免 Web UI 变成远程执行入口）。
