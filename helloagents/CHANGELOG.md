...
- 优化：tool_output 的“详情”展示补充工具/Exit/耗时，并用唯一 id 保证展开/收起不会误操作到其它块。
...
- 新增：右侧工具栏加入“重启监听”按钮（stop→start 一键重启）。
- 修复：重启监听/切换文件触发回放时，服务端按消息 id 去重，避免历史消息重复追加。
- 优化：复制交互统一：各内容块支持长按复制；复制按钮从正文块移到时间戳行（更简洁）。
