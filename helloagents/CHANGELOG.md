...
- UI：导出按钮角标对齐（精简=闪电、译文=地球、组合可同时显示）；导出设置弹层修复 z-index 并移除冗余“当前会话”信息；导出设置支持长按再次关闭，文案收敛为“精简/译文”，弹层宽度进一步收窄，并优先与会话管理抽屉左边框对齐（若遮挡导出按钮则改为右对齐导出按钮左边缘）；同时移除导出设置/监听切换（含标签栏×关闭监听）等操作 toast。
