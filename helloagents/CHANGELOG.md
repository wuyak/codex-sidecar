...
- UI：子代理（subagent）会话分层展示：底部会话标签栏不再被子代理“刷屏”；会话管理抽屉为含子代理的会话增加“折叠/展开”按钮（第 5 个），展开后子会话以标准条目展示并复用 4 个操作按钮管理；主视图“子代理切换条”改为极简圆按钮并保持 sticky（滚动中也能切换）；当主会话“关闭监听/隐藏”时，子会话会自动跟随进入“关闭监听”列表，避免打破折叠暴露在监听名单中。
