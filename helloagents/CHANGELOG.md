...
## [Unreleased]
- UI：统一 hover 提示来源为 `aria-label`（移除 `title`/hover toast 造成的重复提示），精简显示/会话管理/导出提示对齐整改文档。
...
- 修复：会话管理“清除会话”不再跨 sidecar 进程重启永久隐藏（按 pid 绑定）；重启后会自动恢复可见。
...
- 修复：进程跟随模式不再扫描 `sessions` 补齐会话，避免重启误跟历史会话
