...
- UI：会话切换改为右侧中部“会话书签轨”（1~6 个，圆形书签，不挤压对话区）；鼠标悬停或键盘聚焦进入书签轨时弹出全量会话列表面板（覆盖式，离开自动收起），列表展示短时间戳（MM-DD HH:MM）+ shortId，hover 提示完整时间戳 + shortId。
- UI：会话书签轨弹出面板视觉优化：更窄/更轻的阴影与 padding，增加指示箭头；弹出面板打开时隐藏 rail tooltip，避免遮挡与“很丑”的叠层效果。
- UI：会话书签轨样式改为“书签标签”造型（非圆形按钮），并在“全部会话被移除/隐藏”时仍保留可见入口，避免书签轨消失导致无法恢复。
