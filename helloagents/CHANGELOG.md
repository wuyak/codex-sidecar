...
- 改进：导出 Markdown 的“详情”折叠内容改用 HTML `<pre><code>` 渲染，提升 Typora 下代码块折叠兼容性（避免折叠失效导致内容外漏）。
- UI：工具输出中的 diff 高亮对比增强（更明显的红/绿底色），便于快速扫读变更行。
- 修复：点击代码块展开/收起详情改为直接切换（不触发按钮 click/不自动滚动页面），提升响应速度并避免页面“越点越上移”。
- 修复：代码块快速连续点击（浏览器判定为双击）时仍可稳定展开/收起，不再被文本选中逻辑拦截。
