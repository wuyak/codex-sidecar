...
- UI：新增“导入对话”入口（右侧工具栏）；弹窗内支持输入 `rel`（可直接粘贴完整路径自动提取）或按 `sessions/YYYY/MM/DD` 的年/月/日分级浏览（正序）选择 `rollout-*.jsonl` 加入展示并打开；年/月采用分段底座，日期 chips 用右上角角标显示数量，避免“日期+数字”挤在同一行造成干扰。
