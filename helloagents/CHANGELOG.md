...
- 改进：会话标签栏短名称不再误省略，标签宽度更节省，悬停提示补齐
...
- UI：会话管理抽屉从左下改为右下（与标签栏对齐）；标签宽度按“短/中/长”三档自适应；设置页新增“翻译设置”快捷入口；翻译设置抽屉加宽并禁用横向滚动；NVIDIA `Max Tokens` 固定为 8192（仅展示不可修改）；思考翻译状态文案改为“已翻译”。
