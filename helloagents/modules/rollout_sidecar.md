...
- 代码分层：`watch/rollout_watcher.py` 聚焦主流程（`watcher.py` 仅作为向后兼容的 facade）；`watch/*` 承载“跟随策略/进程扫描/解析/去重/tail/翻译队列”等可复用组件
  ...
  - 子代理会话（subagent）：由于 watcher 可能只 tail 文件尾部而错过首行 `session_meta`，`watch/session_meta.py` 会在首次遇到新文件时读取文件头部少量行，提取 `session_meta.payload.source` 并归一化为 `source_kind/parent_thread_id/subagent_depth`，随后附加到每条消息上，供 UI 做父子会话分层展示（避免子代理会话在标签栏中“刷屏”）。
