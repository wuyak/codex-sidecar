...
- **浏览器通信:** 现有 SSE `/events`（可复用），或为 SDK 单独提供 SSE（推荐）
...
flowchart LR
  ...
  UI -->|控制输入/会话| Py
  Py -->|HTTP(本机)| Node[Codex SDK Bridge (Node)]
  Node -->|Codex SDK| Codex[Codex 本机执行/Sandbox]
  Node -->|SSE events| Py
  Py -->|/events 聚合| UI
```

> 注：上图为“推荐拓扑（方案A）”，核心目的是让 UI 仍保持同源访问，避免跨域与多事件源割裂。

## 架构决策 ADR

### ADR-001: 采用 Node 子服务承载 Codex SDK（推荐）
**上下文:** 当前服务端是 Python 标准库实现；Codex SDK 文档以 Node 包形式提供，并要求 Node.js ≥ 18。  
**决策:** 新增 `src/codex-sdk/` 作为 Node 子服务（bridge），由它直接调用 Codex SDK；Python 侧提供同源 API 与 UI 资源，并按需代理/聚合。  
**理由:**
...
**影响:** 引入 Node 运行时与第二进程；需加强本机安全边界与启动脚本管理。

## API 设计（建议稿）

### 线程
- `POST /api/sdk/thread/start`
  - **描述:** 新建 thread 并返回 threadId
  - **响应:** `{ ok: true, threadId: string }`

- `POST /api/sdk/thread/resume`
  - **描述:** 通过 threadId 恢复 thread
  - **输入:** `{ threadId: string }`
  - **响应:** `{ ok: true, threadId: string }`

### 对话与执行
- `POST /api/sdk/turn/start`
  - **描述:** 在指定 thread 上执行一次用户输入（一次 turn）
  - **输入:** `{ threadId: string, text: string }`
  - **响应（MVP）:** `{ ok: true, outputText: string }`
  - **响应（增强）:** `{ ok: true, turnId: string }`（由 SSE 推送流式输出）

- `POST /api/sdk/turn/interrupt`
  - **描述:** 中断当前 turn（需要 SDK/bridge 支持）
  - **输入:** `{ threadId: string, turnId: string }`

### 事件流（可选但推荐）
- `GET /api/sdk/events?threadId=...`
  - **描述:** SSE 推送 SDK 事件（agent 输出 delta、工具调用、计划更新、turn completed 等）
  - **策略:** Python 聚合转发（同源）或 Node 直出再由 Python 反代
...
- **部署/运行:**
  - 在 `ui.sh` 里“一键启动”Python sidecar + Node bridge（默认 bridge 可不启用，或按配置启用）
